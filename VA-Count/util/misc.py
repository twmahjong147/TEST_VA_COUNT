...
def load_model_FSC(args, model_without_ddp):
    if args.resume:
        if args.resume.startswith('https'):
            ...
            # Newer PyTorch versions default to weights_only=True which disallows
            # loading arbitrary pickled objects. For older checkpoints we need to
            # explicitly allow full loading. Try the newer kwarg first and fall
            # back if the installed torch doesn't support it.
            try:
                checkpoint = torch.load(args.resume, map_location='cpu', weights_only=False)
            except TypeError:
                # torch.load doesn't accept weights_only param (older torch)
                checkpoint = torch.load(args.resume, map_location='cpu')
